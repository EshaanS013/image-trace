...
def exif_bytes(index: int, timestamp: datetime, lat: float | None, lon: float | None) -> bytes:
    ...
    exif = {
        "0th": {piexif.ImageIFD.Make: camera[0], piexif.ImageIFD.Model: camera[1], piexif.ImageIFD.Software: "Synthetic Studio" if index == 4 else "IMAGE TRACE Fixture Generator"},
        "Exif": {piexif.ExifIFD.DateTimeOriginal: timestamp.strftime("%Y:%m:%d %H:%M:%S"), piexif.ExifIFD.DateTimeDigitized: timestamp.strftime("%Y:%m:%d %H:%M:%S"), piexif.ExifIFD.LensModel: "Synthetic 35mm", piexif.ExifIFD.ISOSpeedRatings: 100 + index},
...
def generate() -> None:
    ...
    entries = []
    coords: list[tuple[float, float] | None] = [(12.9716 + i * 0.005, 77.5946 + i * 0.004) for i in range(14)] + [None] * 6
    ...
    manifest = {"generator_version": GENERATOR_VERSION, "synthetic_only": True, "coordinate_policy": "Synthetic coordinates for testing; not tied to depicted people or events.",
                "expected": {"image_count": 20, "gps_count": 14, "camera_groups": 2, "editing_software_examples": 1,
                             ...
                             "near_duplicate": ["synthetic-03.jpg", "synthetic-20.jpg"], "minimum_finding_rules": ["TIMESTAMP_SOURCE_FALLBACK", "EDITING_SOFTWARE_PRESENT", "IMPLAUSIBLE_TRAVEL", "MISSING_GPS", "CAMERA_SOURCE_CHANGE"]},
                "files": entries}
